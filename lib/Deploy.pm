...
sub local_install
{
	...
	if ($verbose) {
	    print "Installing from '$file' with no directory ...\n";
	}
    }
    if (! -f $infile) {
	croak "File '$infile' not found";
    }
    if (! $options{nomastercheck}) {
	if ($verbose) {
	    print "Checking for location warning in master file...\n";
	}
	my $text = read_text ($infile);
	my $match = $infile;
	$match =~ s!/usr/home/!/home/!;
	if ($text !~ /\Q$match/) {
	    carp "Add note on location of master file to '$infile'";
	}
	else {
	    if ($verbose) {
		print "OK: Master file has warning about its location.\n";
	    }
	}
    }
    else {
	if ($verbose) {
	    print "\$options{nomastercheck} is true, not doing location check.\n";
	}
    }
    my $outdir = '/home/ben/bin';
    if ($options{outdir}) {
	$outdir = $options{outdir};
	if ($verbose) {
	    print "Installing into specified directory '$outdir'...\n";
	}
	if (! -d $outdir) {
	    croak "Directory $outdir doesn't exist";
	}
    }
    else {
	if ($verbose) {
	    print "Installing into default directory '$outdir'...\n";
	}
    }
    my $outfile = "$outdir/$file";
    if (-f $outfile) {
	if ($verbose) {
	    print "Checking the file permissions of the output file...\n";
	}
	my @stat = stat ($outfile);
	my $omode = $stat[2] & 0777;
	if ($omode != $mode) {
	    # https://www.perlmonks.org/index.pl?node_id=288125
	    carp (sprintf  ("$outfile: bad mode %o should be %o",
			    $omode, $mode));
	}
	else {
	    if ($verbose) {
		print "OK: Copied file permissions look OK.\n";
	    }
	    if ($verbose) {
		print "Checking if the in and out files differ...\n";
	    }	    
	    my $diff = `diff $infile $outfile`;
	    if (! $diff) {
		if ($verbose) {
		    print "OK: Contents of '$outfile' are the same, nothing to do.\n";
		}
		return;
	    }
	    if ($verbose) {
		print "Files differ, continuing.\n";
	    }	    
	}
	if ($verbose) {
	    print "Changing the permissions on $outfile to writeable...\n";
	}
	chmod 0644, $outfile or croak "chmod 0644, $outfile failed: $!";
    }
    if ($verbose) {
	print "Copying the master file...\n";
    }
    copy $infile, $outfile or croak "copy $infile, $outfile failed: $!";
    if ($verbose) {
	print "Setting permissions...\n";
    }
    chmod $mode, $outfile or croak "chmod $mode, $outfile failed: $!";
    if ($verbose) {
